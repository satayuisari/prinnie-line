// จับรางวัลสมาชิก — วันที่ 2 และ 17 เวลา 20:00 Bangkok (ส่งถึงผู้ได้รับ + ประกาศทุกคนพร้อมกัน)
...
function start() {
  // วันที่ 2 และ 17 ของทุกเดือน 20:00 Bangkok — รอบละ 15 วัน
  // เดิม 09:00: ผู้ได้รับได้ข้อความตอนเช้า ก่อนเวลาประกาศทางการตอนค่ำ → สับสน (17 ก.ย. 69)
  // ย้ายมา 20:00 ให้ข้อความถึงผู้ได้รับและประกาศทุกคนออกพร้อมกัน
  // ต้องตรงกับที่บอกไว้ในโพสต์กติกา ไม่งั้นสัญญากับลูกค้าไม่เป็นจริง
  cron.schedule('0 20 2,17 * *', () => runMonthlyPick(), { timezone: 'Asia/Bangkok' });
  const state = process.env.LOYALTY_ENABLED === 'true'
    ? '(ENABLED)' : '(ปิดอยู่ — ตั้ง LOYALTY_ENABLED=true)';
  console.log(`[ดวงเลือกคุณ] คัดผู้ได้รับทุก 15 วัน (วันที่ 2 และ 17) · 20:00 Bangkok · สมาชิกครบ ${pick.MIN_MEMBER_DAYS} วัน ${state}`);
}
